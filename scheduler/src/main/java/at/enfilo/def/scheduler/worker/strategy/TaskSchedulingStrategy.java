...
implements ITaskSchedulingStrategy {
	 ...
	 *
	 * @param tasks - number of tasks of active job to schedulePrograms. nextWorkerId() will be called 'tasks' times.
	 */
	 ...
	 */
	public abstract String nextWorkerId(List<String> rIds) throws TaskOperationException ;
	...
	throws NodeCommunicationException {
		...
		if (!jobs.contains(jId)) {
			LOGGER.warn("Job with id {} is not known by this scheduler. Ignoring request.", jId);
		} else {
			jobs.remove(jId);
	...
	throws TaskOperationException {
		LOGGER.debug("Try to schedulePrograms Task {} on Worker {}.", task.getId(), nodeId);
		try {
			...
			throw new TaskOperationException(e);
		} catch (InterruptedException e) {
			LOGGER.error("Error during scheduling Task {} on Worker {}. Interrupted.", task.getId(), nodeId, e);
			Thread.currentThread().interrupt();
			throw new TaskOperationException(e);
		}
	}
	...
	@Override
	public void abortTask(String wId, String tId) throws TaskOperationException {
		try {
			IWorkerServiceClient client = getNodeClient(wId);
			Future<Void> future = client.abortTask(tId);
			future.get(); // Wait for ticket
			LOGGER.info(DEFLoggerFactory.createTaskContext(tId), "Aborting task successfully executed on worker {}.", wId);
		} catch (ExecutionException | UnknownNodeException | ClientCommunicationException e) {
			LOGGER.error(DEFLoggerFactory.createTaskContext(tId), "Error while aborting task on worker {}.", wId, e);
			throw new TaskOperationException(e);
		} catch (InterruptedException e) {
			LOGGER.error(DEFLoggerFactory.createTaskContext(tId), "Error while aborting task on worker {}. Interrupted.", wId, e);
			Thread.currentThread().interrupt();
			throw new TaskOperationException(e);
		}
	 ...
	 */
	protected List<String> getWorkers(List<String> rIds) throws TaskOperationException {
		if(rIds == null || rIds.isEmpty()) {
			List<String> mNodes = getNodes();
			if(mNodes == null || mNodes.isEmpty()) {
				throw new TaskOperationException("Could not schedule task - no registered node found.");
			}
			return mNodes;
		} else {
		    try {
                ...
                if(mNodes == null || mNodes.isEmpty()) {
                    throw new TaskOperationException("Could not schedule task - no supporting node found.");
                }
                return mNodes;
            } catch (ClientCommunicationException e) {
		        throw new TaskOperationException("Could not schedule task - error communicating with library.");
            }
		}
	}
